...
static void print_file_type_and_permissions(mode_t st_mode) {
    print_filetype(st_mode);
...
// Function to print link count, user, group, and space formatting
static void print_link_user_group(t_file_info *file_info, t_width *width, t_configuration *configuration) {
    mx_printchar(' ');
...
// Function to print device information or human-readable size
static void print_device_or_size(t_file_info *file_info, t_width *width, t_configuration *configuration) {
...
static void print_date_and_time_info(time_t tv_sec, bool full_time_info) {
    mx_printchar(' ');
    print_date_and_time(tv_sec, full_time_info);
    mx_printchar(' ');
}

// Function to print file/directory name and symbolic link information
...
void mx_print_file_info_detailed(t_file_info *file_info, t_width *width, t_configuration *configuration) {
    print_file_type_and_permissions(file_info->stat.st_mode);
    print_extended_acl_markers(file_info);

    print_link_user_group(file_info, width, configuration);

    print_device_or_size(file_info, width, configuration);

    print_date_and_time_info(file_info->timespec.tv_sec, configuration->full_time_info);

    print_file_and_symbolic_link_info(file_info, configuration);

    // Print extended attributes if enabled in configuration and present for the file
    print_extended_attributes_if_enabled(file_info, configuration);

    // Print ACL if enabled in configuration and present for the file
    print_acl_if_enabled(file_info, configuration);
}
